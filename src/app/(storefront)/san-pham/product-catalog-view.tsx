...
import { PackageSearch, Search, ShoppingBag, SlidersHorizontal } from "lucide-react";
...
function ProductCatalogContent({
  ...
  const categoryOptions = useMemo(
    () => [{ slug: "all", name: "Tất cả sản phẩm" }, ...categories],
    [categories]
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
      ...
      {products.length === 0 ? (
        <EmptyState
          icon={PackageSearch}
          title="Chưa có sản phẩm nào được đăng."
          description="Thêm sản phẩm trong Payload Admin (/admin/collections/products) để hiển thị ở đây."
        />
      ) : (
        <>
...
export function ProductCatalogView(props: {
  products: ProductViewModel[];
  categories: ProductCategorySummary[];
}) {
  return (
