...
export default async function ConsultationRequestsPage() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 20, fontWeight: 700, marginBottom: 4 }}>Yêu cầu tư vấn</h1>
      <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 20 }}>
        {requests.length} yêu cầu gần nhất (form /tu-van)
      </p>

      {requests.length === 0 ? (
        <p style={{ color: "#6b7280", fontSize: 13 }}>Chưa có yêu cầu nào.</p>
      ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb", textAlign: "left" }}>
                ...
                <th style={{ padding: "10px 14px" }}>Thắc mắc</th>
              ...
              {requests.map((r) => (
                <tr key={r.id} style={{ borderTop: "1px solid #f0f0f0" }}>
                  <td style={{ padding: "10px 14px" }}>
                    <Link href={`/quan-ly/tu-van/${r.id}`} style={{ color: "#111827", fontWeight: 600, textDecoration: "none" }}>
                      {r.fullName}
                    </Link>
                  </td>
                  ...
                  <td style={{ padding: "10px 14px", color: "#4b5563", maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {r.healthIssue}
                  </td>
