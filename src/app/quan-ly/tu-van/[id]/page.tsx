...
export default async function ConsultationDetailPage({
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <Link href="/quan-ly/tu-van" style={{ fontSize: 12, color: "#2563eb", textDecoration: "none" }}>
        &larr; Danh sách yêu cầu tư vấn
      </Link>
      ...
      <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #e5e7eb", padding: 24, marginTop: 12 }}>
        ...
        <dl style={{ fontSize: 13, display: "grid", gridTemplateColumns: "140px 1fr", rowGap: 8 }}>
          ...
          <dt style={{ color: "#6b7280" }}>Tuổi</dt>
          <dd style={{ margin: 0 }}>{request.age ?? "—"}</dd>
          <dt style={{ color: "#6b7280" }}>Giới tính</dt>
          <dd style={{ margin: 0 }}>{request.gender || "—"}</dd>
        </dl>

        <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #f0f0f0" }}>
          <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 6 }}>Thắc mắc y khoa:</p>
          <p style={{ fontSize: 14, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>{request.healthIssue}</p>
        </div>
      </div>
    </div>
  );
